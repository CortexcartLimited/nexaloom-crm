...
export const CalendarView: React.FC<CalendarViewProps> = ({ interactions, leads, user, onAddInteraction }) => {
  ...
  const handleSubmitSchedule = async (e: React.FormEvent) => {
      ...
      const scheduledTime = `${dateStr}T${newMeeting.time}:00.000Z`;
  ...
  const getEventStyle = (type: Interaction['type']) => {
      switch(type) {
              ...
              >
                ...
                <div className="space-y-1 overflow-hidden">
                  {dayEvents.slice(0, 3).map(event => (
                    <div key={event.id} className={`px-2 py-0.5 rounded text-[10px] font-medium border truncate flex items-center gap-1 shadow-sm ${getEventStyle(event.type)}`}>
