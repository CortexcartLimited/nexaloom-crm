...
export const UserManagementView: React.FC<UserManagementViewProps> = ({ users, currentUser, onAddUser, onUpdateUser, onDeleteUser }) => {
  ...
  const getRoleBadge = (role: UserRole) => {
    switch (role) {
      ...
      case UserRole.RELATIONSHIP_AGENT:
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800">
            <HeartHandshake size={12} /> Support Agent
          </span>
        );
      default:
        return null;
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {[
                                { id: UserRole.ADMIN, label: 'Administrator', icon: Shield, desc: 'Full access to settings, users, and all data.' },
                                { id: UserRole.TEAM_LEAD, label: 'Team Lead', icon: Briefcase, desc: 'Can manage agents and view team performance.' },
                                { id: UserRole.SALES_AGENT, label: 'Sales Agent', icon: Phone, desc: 'Access to leads, pipeline, and tasks.' },
                                { id: UserRole.RELATIONSHIP_AGENT, label: 'Relationship Agent', icon: HeartHandshake, desc: 'Focus on support tickets and customer retention.' },
                            ].map((roleOption) => (
                                <label 
                                    key={roleOption.id} 
                                    className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${formData.role === roleOption.id ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-500 ring-1 ring-blue-500' : 'bg-white dark:bg-gray-700/50 border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                                >
                                    <input 
                                        type="radio" 
                                        name="role" 
                                        ...
                                        onChange={() => setFormData({...formData, role: roleOption.id as UserRole})}
                                        className="mt-1"
                                    />
                                    <div>
