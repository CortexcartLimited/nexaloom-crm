...
export const DialerPanel: React.FC<DialerPanelProps> = ({ isOpen, onClose, userName, phoneNumber, leadId, tenantId, onAddInteraction }) => {
  ...
  const endCall = async () => {
    if (leadId && tenantId && onAddInteraction) {
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">
                {callStatus === 'IDLE' ? 'Nexaloom Dialer' : 
                 ...
                 callStatus === 'IN_CALL' ? 'In Call' : 'Transfer Call'}
              </h2>
              {callStatus === 'IN_CALL' && (
             ...
             <div className="space-y-4 animate-in zoom-in-95 duration-300">
                ...
                <div>
                    <p className="text-xl font-bold text-gray-900 dark:text-white">
                        {callStatus === 'TRANSFERRING' ? (transferNumber || 'Extension') : (dialedNumber || 'Private Number')}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        {callStatus === 'TRANSFERRING' ? 'Transferring...' : 'Active Connection'}
                    </p>
                </div>
             </div>
           )}
