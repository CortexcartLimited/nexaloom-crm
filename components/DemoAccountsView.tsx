...
export const DemoAccountsView: React.FC<DemoAccountsViewProps> = ({ demoAccounts, leads, user, onAddDemo, onDeleteDemo, onAddInteraction }) => {
    ...
    const handleProvision = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // We still use onAddDemo to update the local state in App.tsx for now
            // though App.tsx should ideally refresh all leads.
            await onAddDemo({
                leadId: lead.id,
                leadName: lead.name,
                username: `demo_${lead.id.substring(0, 4)}`, // Fallback for the UI
                passwordHash: '********',
    ...
    const handleTerminate = async (id: string, leadId: string) => {
        if (!window.confirm('Are you sure you want to terminate this demo?')) return;
        setTerminatingId(id);
        try {
            await api.terminateDemo(leadId);
            await onDeleteDemo(id);
        } catch (err: any) {
            alert(err.message || 'Failed to terminate demo');
        } finally {
                                ...
                                <tbody className="divide-y divide-gray-800/50">
                                    {recentOrders.map((o, i) => (
                                        <tr key={i} className="text-sm font-medium hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase ${o.status === 'Shipped' ? 'bg-blue-900/40 text-blue-400' :
