    ...
    router.post('/', async (req, res) => {
        ...
        try {
            ...
            await connection.query(
                `INSERT INTO proposals (id, tenantId, name, leadId, leadName, leadCompany, totalValue, taxRate, taxAmount, isTaxEnabled, status, validUntil, terms, createdBy, currency, createdAt) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
                [proposalId, tenantId, name, leadId, leadName, leadCompany, totalValue, req.body.taxRate || 0, req.body.taxAmount || 0, req.body.isTaxEnabled ? 1 : 0, status, validUntilDate, terms, createdBy, currency || 'GBP']
            );
